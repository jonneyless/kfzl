...
class Daemon:
    ...
    def daemonize(self):
        ...
        def signalTermHandler(signum, frame):  # pylint: disable=W0613
            if self.use_cleanup:
                self.cleanup()
                time.sleep(0.1)
        ...
        if self.use_gevent:
            ...
            gevent.reinit()
            gevent.signal_handler(signal.SIGTERM, signalTermHandler, signal.SIGTERM, None)
            gevent.signal_handler(signal.SIGINT, signalTermHandler, signal.SIGINT, None)
        else:
    ...
    def start(self):
        ...
        try:
            pifFile = open(self.pidfile, 'r', encoding='utf-8')
            pid = int(pifFile.read().strip())
            pifFile.close()
        except IOError:
            pid = None
        except SystemExit:
    ...
    def status(self):
        ...
        """
        daemon_running = self.isRunning()

        if daemon_running:
            message = "pidfile %s found, daemon PID is %d\n"
            sys.stdout.write(message % (self.pidfile, self.getPid()))
        else:
        ...
        if self.verbose >= 1:
            timestamp()
            self.log(f"{__name__} status is: {self.isRunning()}")
    ...
    def getPid(self):
        ...
        """
        try:
            pidFile = open(self.pidfile, 'r', encoding='utf-8')
